...
public record LocalVariableInfo(String name, String desc, String signature, LabelInfo start, LabelInfo end,
    ...
    public LocalVariableInfo(LocalVariableNode node) {
        this(node.name, node.desc, AsmUtils.signatureToJavaCode(node.name, node.desc, node.signature, null), new LabelInfo(node.start), new LabelInfo(node.end), node.index);
    }
